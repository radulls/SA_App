...
const commentApi = axios.create({
  baseURL: 'http://89.108.118.249:5003/api/event-comments',
});
...
export const getCommentsByEvent = async (eventId: string) => {
  const response = await commentApi.get(`/${eventId}`);
  return response.data;
};

// ➕ Создать комментарий (или ответ)
export const createComment = async (eventId: string, text: string, parentCommentId?: string) => {
  const response = await commentApi.post(`/${eventId}`, {
    text,
