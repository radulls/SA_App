...
import { getCommentsByPost, createComment, toggleLikeComment, deleteComment, reportComment } from '@/api/comment';
...
const CommentsBottomSheet: React.FC<CommentsBottomSheetProps> = ({ isVisible, onClose, postId, currentUserId }) => {
  ...
  const handleSend = async () => {
    if (!text.trim()) return;
    try {
      await createComment(postId, text, replyTo?.id);
      setText('');
      setReplyTo(null);
      fetchComments();
    } catch (err) {
