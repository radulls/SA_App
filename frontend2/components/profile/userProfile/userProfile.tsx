...
const UserProfile: React.FC = () => {
  ...
  let userId = '67b50c48e5b0f831afab4218'
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const blockedStatus = await checkIfBlocked(userId);
    ...
    <BottomSheetMenu
      ...
      buttons={[
        ...
        loggedInUser?.role === 'admin' || loggedInUser?.role === 'creator' // Проверяем именно залогиненного пользователя
          ? { label: 'Управление', onPress: () => router.push(`/admin/control/[id]`), icon: null, isRowButton: false }
